...
/// Validator that keeps the input buffer open for multiline JSON.
///
/// When the input starts with `{`, Enter inserts a newline until all
/// braces and brackets are balanced (respecting JSON string quoting).
/// Regular HMP commands are submitted immediately on Enter.
...
impl Validator for JsonValidator {
    fn validate(&self, line: &str) -> ValidationResult {
        ...
        if json_brackets_balanced(trimmed) {
...
/// Check whether all `{}`/`[]` brackets are balanced, skipping
/// characters inside JSON strings (handling `\"` escapes).
fn json_brackets_balanced(s: &str) -> bool {
    let mut depth = 0i32;
...
mod tests {
    ...
    #[test]
    fn balanced_simple() {
        assert!(json_brackets_balanced(r#"{"execute": "query-version"}"#));
    }
    ...
    #[test]
    fn nested_balanced() {
        assert!(json_brackets_balanced(
            r#"{"execute": "device_add", "arguments": {"driver": "e1000"}}"#
        ));
    }
    ...
    fn brackets_in_strings() {
        assert!(json_brackets_balanced(r#"{"key": "val{ue}"}"#));
    }
    ...
    #[test]
    fn array_incomplete() {
        assert!(!json_brackets_balanced(r#"{"list": [1, 2"#));
    }
    ...
    fn multiline_json() {
        ...
        assert!(json_brackets_balanced(input));
    }
    ...
    #[test]
    fn validator_hmp_command() {
        let v = JsonValidator;
        assert!(matches!(
            v.validate("info version"),
            ValidationResult::Complete
        ));
    }

    #[test]
    fn validator_complete_json() {
        let v = JsonValidator;
        assert!(matches!(
            v.validate(r#"{"execute": "query-version"}"#),
            ValidationResult::Complete
        ));
    }

    #[test]
    fn validator_incomplete_json() {
        let v = JsonValidator;
        assert!(matches!(
            v.validate(r#"{"execute": "query-version""#),
            ValidationResult::Incomplete
...
pub async fn run(conn: QmpConnection, registry: &Registry) -> anyhow::Result<ExitReason> {
    ...
    let mut editor = Reedline::create()
        ...
        .with_validator(Box::new(JsonValidator));
