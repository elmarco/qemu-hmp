...
pub async fn cmd_hostfwd_remove(
    ...
    // Match the built-in HMP output exactly:
    // monitor_printf(mon, "host forwarding rule for %s removed\n", src_str);
    Ok(format!("host forwarding rule for {rule} removed\n"))
}
