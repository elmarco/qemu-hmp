...
/// Infer the JSON type for a keyval string value.
///
/// QMP expects typed JSON but QEMU's keyval format is all strings.
/// This mirrors QEMU's property visitor heuristics:
/// - `true`/`false`/`on`/`off` → boolean
/// - Size suffixes (K/M/G/T) or plain integers → number
/// - Everything else → string
fn parse_keyval_value(s: &str) -> serde_json::Value {
    match s {
        "true" | "on" => return serde_json::Value::Bool(true),
        "false" | "off" => return serde_json::Value::Bool(false),
        _ => {}
    }
    if let Ok(n) = crate::args::parse_size(s) {
        return serde_json::json!(n);
    }
    serde_json::Value::String(s.to_string())
}

/// Parse a QEMU keyval string into a typed JSON object.
///
/// The format is: `[implied],key=value,key=value,...`
/// When `implied_key` is `Some`, the first positional value (without `=`)
/// is assigned to that key as a string.  Values in `key=value` pairs are
/// type-inferred via [`parse_keyval_value`].
pub(crate) fn parse_keyval(
    ...
    for (i, part) in input.split(',').enumerate() {
        ...
        if let Some((key, value)) = part.split_once('=') {
            map.insert(key.to_string(), parse_keyval_value(value));
        } else if i == 0 {
...
impl Registry {
    ...
    fn register_handlers(&mut self) {
        ...
        register!(self, info, "virtio", info_virtio::cmd_info_virtio);
        register!(
            self,
            info,
    ...
    pub async fn dispatch(&self, conn: &QmpConnection, line: &str, styled: bool) -> DispatchOutput {
        ...
        if line.starts_with('{') {
            return self.dispatch_raw(conn, line, styled).await;
        }
        ...
        // Built-in: help / ?
        if cmd == "help" || cmd == "?" {
            let topic = if tokens.len() > 2 && tokens[1] == "info" {
                // "help info <subcommand>" → help for that info subcommand
                Some(tokens[2])
    ...
    async fn dispatch_raw(&self, conn: &QmpConnection, line: &str, styled: bool) -> DispatchOutput {
        let json: serde_json::Value = match serde_json::from_str(line) {
            Ok(v) => v,
            Err(e) => return DispatchOutput::Output(format!("Invalid JSON: {e}")),
        };

        match conn.execute_raw(&json).await {
            Ok(resp) => {
                if styled {
                    DispatchOutput::Output(crate::format::json_colored(&resp))
                } else {
                    DispatchOutput::Output(crate::format::json_fallback(&resp))
                }
            }
            Err(e) if e.kind() == std::io::ErrorKind::UnexpectedEof => DispatchOutput::Disconnected,
            Err(e) => DispatchOutput::Output(format!("QMP error: {e}")),
        }
    }
...
mod tests {
    ...
    #[test]
    fn keyval_value_bool() {
        assert_eq!(parse_keyval_value("true"), serde_json::Value::Bool(true));
        assert_eq!(parse_keyval_value("false"), serde_json::Value::Bool(false));
        assert_eq!(parse_keyval_value("on"), serde_json::Value::Bool(true));
        assert_eq!(parse_keyval_value("off"), serde_json::Value::Bool(false));
    }

    #[test]
    fn keyval_value_integer() {
        assert_eq!(parse_keyval_value("42"), serde_json::json!(42));
        assert_eq!(parse_keyval_value("0"), serde_json::json!(0));
        assert_eq!(parse_keyval_value("0xff"), serde_json::json!(255));
    }

    #[test]
    fn keyval_value_size_suffix() {
        assert_eq!(parse_keyval_value("1G"), serde_json::json!(1073741824));
        assert_eq!(parse_keyval_value("512M"), serde_json::json!(536870912));
        assert_eq!(parse_keyval_value("4K"), serde_json::json!(4096));
    }

    #[test]
    fn keyval_value_string() {
        assert_eq!(
            parse_keyval_value("hello"),
            serde_json::Value::String("hello".into())
        );
        assert_eq!(
            parse_keyval_value("mem1"),
            serde_json::Value::String("mem1".into())
        );
    }

    #[test]
    fn keyval_object_add_example() {
        let obj = parse_keyval("memory-backend-ram,id=mem1,size=1G", Some("qom-type")).unwrap();
        assert_eq!(obj["qom-type"], "memory-backend-ram");
        assert_eq!(obj["id"], "mem1");
        assert_eq!(obj["size"], serde_json::json!(1073741824));
    }

    #[test]
    fn keyval_no_implied_key_rejects_positional() {
        assert!(parse_keyval("positional,key=val", None).is_err());
    }

    #[test]
    fn keyval_no_implied_key_accepts_all_kv() {
        let obj = parse_keyval("driver=file,node-name=n1", None).unwrap();
        assert_eq!(obj["driver"], "file");
        assert_eq!(obj["node-name"], "n1");
    }

    #[test]
    fn help_info_subcommand_shows_specific_help() {
        let registry = Registry::new();
        let output = registry.help(Some("version"), false);
        assert!(
            output.contains("info version"),
            "expected info version help, got: {output}"
        );
        assert!(
            !output.contains("Available info subcommands"),
            "should not list all info subcommands"
        );
    }

    #[test]
    fn help_info_lists_all_subcommands() {
        let registry = Registry::new();
        let output = registry.help(Some("info"), false);
        assert!(output.contains("Available info subcommands"));
    }

    #[test]
    fn help_info_unknown_subcommand() {
        let registry = Registry::new();
        let output = registry.help(Some("nosuchinfocmd"), false);
        assert!(
