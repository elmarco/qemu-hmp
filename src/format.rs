...
/// Format a QMP event for display.
///
/// Serializes the event to JSON to uniformly extract the event name,
/// data fields, and timestamp — avoiding a giant match on every
/// variant of the generated `Event` enum.
///
/// Output looks like:
/// ```text
/// ✨ DEVICE_TRAY_MOVED { device: "ide1-cd0", tray_open: true } [14:32:48.230]
/// ```
pub fn format_event(event: &Event) -> String {
    // The Event enum uses #[serde(tag = "event")], so it serializes to:
    // {"event": "NAME", "data": {...}, "timestamp": {"seconds": N, ...}}
...
mod tests {
    ...
    fn event_format_with_data() {
        // Event uses #[serde(tag = "event")] — internally tagged.
        let secs: u64 = 45045;
        let us: u64 = 230000;
