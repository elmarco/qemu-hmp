...
pub fn parse_args(input: &[&str], defs: &[ArgDef]) -> Result<HashMap<String, ArgValue>, String> {
    ...
    for def in defs {
        ...
        let value = match &def.arg_type {
            ...
            ArgType::Long => {
                // Long type consumes all remaining tokens as a single
                // expression string.  This matches QEMU's get_expr()
                // which reads the full remainder of the command line.
                let expr = if pos < remaining.len() {
...
mod tests {
    ...
    #[test]
    fn long_consumes_remaining_tokens() {
        // Long type should consume all remaining tokens as one expression.
